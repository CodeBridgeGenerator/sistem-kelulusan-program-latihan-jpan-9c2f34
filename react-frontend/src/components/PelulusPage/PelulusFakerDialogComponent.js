...
import pelulusFakerFactory from "./pelulusFakerFactory";
...
const PelulusFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = pelulusFakerFactory(props.user,fakerCount,nameIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("pelulus").create(elem));
...
export default connect(mapState, mapDispatch)(PelulusFakerDialogComponent);
