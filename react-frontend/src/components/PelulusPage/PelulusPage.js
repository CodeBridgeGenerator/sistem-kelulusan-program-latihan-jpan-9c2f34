...
import PelulusDatatable from "./PelulusDataTable";
...
import PelulusFakerDialogComponent from "./PelulusFakerDialogComponent";
import PelulusSeederDialogComponent from "./PelulusSeederDialogComponent";

const PelulusPage = (props) => {
    ...
    const filename = "pelulus.csv";

    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("pelulus");
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("pelulus").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Pelulus", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service("pelulus").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">Pelulus </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                    <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="pelulus-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="pelulus-datatable">
                    <PelulusDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>
            ...
            <PelulusEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <PelulusCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <PelulusFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <PelulusSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(PelulusPage);
