...
const PelulusDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const dropdownTemplate0 = (rowData, { rowIndex }) => <p >{rowData.name?.name}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "pelulus"});
...
<Column field="name" header="Name" body={dropdownTemplate0} filter={selectedFilterFields.includes("name")} hidden={selectedHideFields?.includes("name")}  style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Pelulus Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
          serviceName="pelulus"            
          onUploadComplete={() => {
      ...
      <Dialog header="Search Pelulus" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default PelulusDataTable;